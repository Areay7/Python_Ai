...
class ImageOverlayApp(QWidget):
    def __init__(self):
        ...
        self.offsets = {}  # 存储偏移值的字典
        self.match_rules = {}  # 存储匹配规则的字典
    ...
    def importFromTxt(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "警告", f"导入txt文件时出错：{str(e)}")
    ...
    def overlayImages(self):
        ...
        files_to_overlay = [filename for filename in os.listdir(self.folder_path)]
        files_to_overlay.sort()  # 排序文件列表，以确保同名文件相邻

        # 在开始叠加时存储匹配规则和偏移值到字典中
        self.storeMatchRulesAndOffsets()

        # 统计需要叠加的文件数量
        total_files = len(files_to_overlay)
        files_to_process = total_files // 2 + 1  # 需要叠加的文件数量为总文件数量的一半加1

        # 计数器，记录已经叠加的文件数量
        processed_count = 0

        for i in range(len(files_to_overlay) - 1):
            first_image_name, first_image_ext = os.path.splitext(files_to_overlay[i])
            second_image_name, second_image_ext = os.path.splitext(files_to_overlay[i + 1])
            ...
            # 检查是否已经处理过该文件
            if first_image_name in processed_files:
                continue

            first_image_path = os.path.join(self.folder_path, files_to_overlay[i])
            second_image_path = os.path.join(self.folder_path, files_to_overlay[i + 1])

            # 获取偏移值
            x_offset, y_offset = self.getOffsets(first_image_name)

            # 构建输出文件路径，将第一张图片的文件名作为输出文件名
            output_file = f"{first_image_name}_覆盖_{os.path.basename(second_image_path)}"
            ...
            thread = threading.Thread(target=self.processImages,
                                      args=(second_image_path, first_image_path, x_offset, y_offset, output_file_path))
            thread.start()
            self.threads.append(thread)

            # 将已经完成叠加的文件名添加到集合中
            processed_files.add(first_image_name)
            processed_count += 1
    ...
    def storeMatchRulesAndOffsets(self):
        for match_input, x_input, y_input in self.input_layouts:
            ...
            # 检查是否有匹配规则和偏移值输入
            if match_text and x_offset and y_offset:
                # 将匹配规则和偏移值存储到字典中
                self.match_rules[match_text] = (float(x_offset), float(y_offset))

    def getOffsets(self, image_name):
        # 获取不含后缀的文件名
        base_image_name = os.path.splitext(image_name)[0]
        # 遍历匹配规则字典，寻找与文件名匹配的规则
        for match_rule, offsets in self.match_rules.items():
            if match_rule in image_name:
                return offsets
        self.output_text.append(f'未找到匹配字符"{image_name}"的偏移值')
        return 0.0, 0.0  # 如果未找到匹配的偏移值，则默认返回 (0.0, 0.0)
