...
class FileRenameApp(QWidget):
    ...
    def modifyFileNames(self, add_prefix=True):
        ...
        for i in range(self.file_list.count()):
            ...
            # 处理尺码为S的情况
            if 'S' in file_name and not file_name.startswith('S'):
                s_index = file_name.index('S')
                if add_prefix:
                    if file_name[s_index - 1] != '-':
                        file_name = file_name[:s_index] + '-' + file_name[s_index:]
                else:
                    if file_name[s_index - 1] == '-':
                        file_name = file_name[:s_index - 1] + file_name[s_index:]
